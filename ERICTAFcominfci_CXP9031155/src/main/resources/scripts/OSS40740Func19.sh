...
function createNode()
{
 ...
 l_cmd=`/opt/ericsson/arne/bin/import.sh -import -i_nau -f /tmp/OSS40740Func19_create.xml | tee /tmp/create_log.txt`
 l_cmd1=`sed -n -e '/Import Finished/p' -e '/No Errors/p' /tmp/create_log.txt`
 ret=$?
 if [ $ret != 0 ]
 then
		G_PASS_FLAG=1
 		log "FAILED:: INPUT XML file /tmp/OSS40740Func19_create.xml is not correct"
 		
 else
        log "SUCESS:: INPUT XML file /tmp/OSS40740Func19_create.xml is correct and NODE is added sucessfully"
fi
...
function directoriesCheck () {
	...
	DIR_MASTER=/var/opt/ericsson/smrsstore/GRAN/nedssv4/GRAN_STN_TC01
	...
	DIR_OTHER=/export/GRAN/nedssv4/GRAN_STN_TC01
	...
	l_cmd2=`ssh smrs_master ssh -o StrictHostKeyChecking=no nedss ls -A $DIR_OTHER/NePersistent && echo "Not Empty" || echo "Empty"`
		if [[ $l_cmd == "Not Empty" ]] ; then
			log "INFO:: $DIR_OTHER/NePersistent exists"
		else
			G_PASS_FLAG=1
	...
	l_cmd3=`ssh smrs_master ssh -o StrictHostKeyChecking=no nedss ls -A $DIR_OTHER/NeTransientUp/PM && echo "Not Empty" || echo "Empty"`
		if [[ $l_cmd == "Not Empty" ]] ; then
			log "INFO:: $DIR_OTHER/NeTransientUp/PM exists"
		else
			G_PASS_FLAG=1
...
function deleteNode()
{
 ...
 l_cmd=`/opt/ericsson/arne/bin/import.sh -import -i_nau -f /tmp/OSS40740Func19_delete.xml | tee /tmp/delete_log.txt`
 l_cmd1=`sed -n -e '/Import Finished/p' -e '/No Errors/p' /tmp/delete_log.txt`
 ret=$?
 if [ $ret != 0 ]
 then
		G_PASS_FLAG=1
 		log "FAILED:: INPUT XML file /tmp/OSS40740Func19_delete.xml is not correct"
 else
		sleep 120
		l_cmd2=`/opt/ericsson/arne/bin/export.sh -f /var/tmp/1.txt`
		l_cmd3=`grep GRAN_STN_TC01 /var/tmp/1.txt`
		ret=$?
		if [ $ret != 0 ] ; then
