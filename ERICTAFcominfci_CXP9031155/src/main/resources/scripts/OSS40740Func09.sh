...
function sftpslaveService ()
{
        ...
        l_count=0
	log "Checking sftp for list ${sftpList[@]}"
        while [ $l_count -lt $sftpCount ]; do
			prepareSftp ${sftpList[$l_count]}
            createExpect  $INPUTEXP $OUTPUTEXP "$EXPCMD"
            executeExpect $OUTPUTEXP
            if [ $? == 0 ]; then
                log "SUCCESS:$FUNCNAME::sftp is of FTP user ${sftpList[$l_count]} is successfull"
            else
				G_PASS_FLAG=1
				log "ERROR:$FUNCNAME::sftp of ${sftpList[$l_count]} is failed"
            fi
                let l_count+=1
                done
}
...
function executeAction () 
{
	...
	if [ $l_action == 9 ]; then
		log "INFO:$FUNCNAME::checking sftp of FTP users"
		sftpslaveService
...
#checking sftp of FTP users
log "ACTION 9 Started"
executeAction 9
log "ACTION 9 Completed"
