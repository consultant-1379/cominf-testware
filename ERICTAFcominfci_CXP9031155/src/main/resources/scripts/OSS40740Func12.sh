...
if [ $1 = "smrs_master" ] ; then
        ...
        l_cmd=`ssh smrs_master -o StrictHostKeyChecking=no ssh nedss df -h | grep nedss >/var/tmp/mounts.txt`
...
function executeAction () 
{
	...
	if [ $l_action == 12 ]; then
		log "INFO:$FUNCNAME::verifying the Network directories are mounted on /export folder on SMRS MASTER "
		networkMounts "smrs_master"
	fi
...
log "ACTION 12 Started"
executeAction 12
log "ACTION 12 Completed"
