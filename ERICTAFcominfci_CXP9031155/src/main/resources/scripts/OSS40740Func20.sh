...
function createNode()
{
 ...
 l_cmd=`/opt/ericsson/arne/bin/import.sh -import -i_nau -f /tmp/OSS40740Func20_create.xml | tee /tmp/create_log.txt`
 l_cmd1=`sed -n -e '/Import Finished/p' -e '/No Errors/p' /tmp/create_log.txt`
 ret=$?
 if [ $ret != 0 ]
 then
                G_PASS_FLAG=1
                log "FAILED:: INPUT XML file /tmp/OSS40740Func20_create.xml is not correct"

 else
        log "SUCESS:: INPUT XML file /tmp/OSS40740Func20_create.xml is correct and NODE is added sucessfully"
fi
...
function directoriesCheck () {
        ...
        if [ -d /var/opt/ericsson/smrsstore/GRAN/CommonPersistent/BackupCM/LANSwitch/GRAN_LANSwitch_TC01 ] ; then
        ...
        l_cmd=`ssh smrs_master ls -A /export/GRAN/CommonPersistent/BackupCM/LANSwitch/GRAN_LANSwitch_TC01 && echo "Not Empty" || echo "Empty"`
        ...
        l_cmd2=`ssh smrs_master ssh -o StrictHostKeyChecking=no nedss ls -A /export/GRAN/CommonPersistent/BackupCM/LANSwitch/GRAN_LANSwitch_TC01 && echo "Not Empty" ||
echo "Empty"`
                if [[ $l_cmd == "Not Empty" ]] ; then
                        log "INFO:: $DIR_OTHER/NePersistent exists"
                else
...
function deleteNode()
{
 ...
 l_cmd=`/opt/ericsson/arne/bin/import.sh -import -i_nau -f /tmp/OSS40740Func20_delete.xml | tee /tmp/delete_log.txt`
 l_cmd1=`sed -n -e '/Import Finished/p' -e '/No Errors/p' /tmp/delete_log.txt`
 ret=$?
 if [ $ret != 0 ]
 then
                G_PASS_FLAG=1
                log "FAILED:: INPUT XML file /tmp/OSS40740Func20_delete.xml is not correct"
 else
                sleep 120
                l_cmd2=`/opt/ericsson/arne/bin/export.sh -f /var/tmp/1.txt`
                l_cmd3=`grep GRAN_LANSwitch_TC01 /var/tmp/1.txt`
                ret=$?
                if [ $ret != 0 ] ; then
