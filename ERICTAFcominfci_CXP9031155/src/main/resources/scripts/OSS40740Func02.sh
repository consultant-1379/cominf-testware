...
## TC TAFTM Link :http://taftm.lmera.ericsson.se/#tm/viewTC/1464
##TC VARIABLE##
...
PERFORM_ARNEIMPORTS=yes' > $G_MASTER_TEMPLATE
}
...
function ConfigSmrsMaster1 ()
{
   prepareMasterTemplate
	EXPCMD="$G_CONFIG_SCRIPT add smrs_master -f $G_MASTER_TEMPLATE"
    EXITCODE=5
    INPUTEXP=/tmp/${SCRIPTNAME}.in
    OUTPUTEXP=/tmp/${SCRIPTNAME}.exp
...
function checkingSmoFtp () 
{

		OSS_ALIAS_NAME=`grep -w "OSS_ALIAS" $smrsConfig | awk -F'=' '{print $2}'`
		if [ ! -z $OSS_ALIAS_NAME ] ; then
			CHECK_SMO_FTP_SERVICE_MASTER "$OSS_ALIAS_NAME" "y" "y" "y" "y" "y"
				
				ret=$?
				echo $ret
				if [ $ret != 0 ] ; then
					G_PASS_FLAG=1
					log "ERROR::$FUNCNAME: SmoFtp Service doesn't exists"
				else
					log " SUCCESS::$FUNCNAME: SmoFtp Service exists "
				fi
		else 
...
function executeAction () 
{
	...
	if [ $l_action == 2 ]; then  
		...
		log "INFO:$FUNCNAME::Checking SmoFtp Service exists"
		checkingSmoFtp
	fi
...
#Configuring smrs_master with ARNE imports and checking for SmoFtp serivces
