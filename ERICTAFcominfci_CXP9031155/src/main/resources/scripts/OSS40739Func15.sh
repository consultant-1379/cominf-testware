...
## TC TAFTM Link :http://taftm.lmera.ericsson.se/#tm/viewTC/4689
###TC VARIABLE##
        ...
        if [[ "$l_cmd" = "YES" &&  "$l_cmd1" = "YES" ]]  ; then
                log "INFO:Checking the scritp /ericsson/ocs/bin/setup_dns.bsh is executed or not"
                        if [[ `ls -l /ericsson/ocs/log/setup_dns.bsh*` ]] ; then
                                log "INFO: Checking $NAMED/named.ca file existance"
                                if [[ -f $NAMED/named.ca  ]] ; then
                                        log "INFO: $NAMED/named.ca file exits"
                                else
                                        G_PASS_FLAG=1
                                        log "ERROR: $NAMED/named.ca file doesnot exists"
                                fi
                        else
                                G_PASS_FLAG=1
...
function executeAction ()
{
 ...
 if [ $l_action == 1 ]; then
   log "INFO: Testcase regarding the TR:HR36132"
   log "INFO:$FUNCNAME::Checking the file $NAMED/named.ca exists"
   fileExistance
...
#Checking the file $NAMED/named.ca exists
executeAction 1
